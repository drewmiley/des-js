...
<p>This example shows how d3fc can be used to render dynamic data. The basic principle is that the chart render function
...
<span class="hljs-keyword">var</span> generator = fc.data.random.financial();
<span class="hljs-keyword">var</span> data = generator(<span class="hljs-number">110</span>);
...
<span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">renderChart</span><span class="hljs-params">()</span> </span>{
    ...
    <span class="hljs-keyword">var</span> datum = generator(<span class="hljs-number">1</span>)[<span class="hljs-number">0</span>];
    data.push(datum);
    data.shift();

    <span class="hljs-comment">// compute the bollinger bands</span>
    <span class="hljs-keyword">var</span> bollingerAlgorithm = fc.indicator.algorithm.bollingerBands();
    bollingerAlgorithm(data);

    <span class="hljs-comment">// Offset the range to include the full bar for the latest value</span>
    <span class="hljs-keyword">var</span> DAY_MS = <span class="hljs-number">1000</span> * <span class="hljs-number">60</span> * <span class="hljs-number">60</span> * <span class="hljs-number">24</span>;
    <span class="hljs-keyword">var</span> xExtent = fc.util.extent()
        .fields(<span class="hljs-string">'date'</span>)
        .padUnit(<span class="hljs-string">'domain'</span>)
        .pad([DAY_MS * -bollingerAlgorithm.windowSize()(data), DAY_MS]);

    <span class="hljs-comment">// ensure y extent includes the bollinger bands</span>
    <span class="hljs-keyword">var</span> yExtent = fc.util.extent().fields([
        <span class="hljs-function"><span class="hljs-keyword">function</span><span class="hljs-params">(d)</span> </span>{ <span class="hljs-keyword">return</span> d.bollingerBands.upper; },
        <span class="hljs-function"><span class="hljs-keyword">function</span><span class="hljs-params">(d)</span> </span>{ <span class="hljs-keyword">return</span> d.bollingerBands.lower; }
    ]);
    ...
    <span class="hljs-keyword">var</span> chart = fc.chart.cartesian(
     ...
     .margin({left: <span class="hljs-number">30</span>, right: <span class="hljs-number">30</span>, bottom: <span class="hljs-number">20</span>, top: <span class="hljs-number">30</span>});

    <span class="hljs-comment">// obtain ticks from the underlying scales</span>
    <span class="hljs-keyword">var</span> xTicks = chart.xScaleTicks(<span class="hljs-number">10</span>);
    <span class="hljs-keyword">var</span> yTicks = chart.yScaleTicks(<span class="hljs-number">10</span>);

    <span class="hljs-comment">// render a reduced number of ticks on each axis</span>
    chart.xTickValues(xTicks.filter(<span class="hljs-function"><span class="hljs-keyword">function</span><span class="hljs-params">(d)</span> </span>{ <span class="hljs-keyword">return</span> d.getDate() % <span class="hljs-number">2</span> === <span class="hljs-number">0</span>; }))
        .yTickValues(yTicks.filter(<span class="hljs-function"><span class="hljs-keyword">function</span><span class="hljs-params">(d, i)</span> </span>{ <span class="hljs-keyword">return</span> i % <span class="hljs-number">2</span> === <span class="hljs-number">0</span>; }));

    <span class="hljs-comment">// Create the gridlines and series</span>
    <span class="hljs-keyword">var</span> gridlines = fc.annotation.gridline()
        .xTickValues(xTicks)
        .yTickValues(yTicks);
    <span class="hljs-keyword">var</span> candlestick = fc.series.candlestick();
    <span class="hljs-keyword">var</span> bollingerBands = fc.indicator.renderer.bollingerBands();
    ...
    d3.select(<span class="hljs-string">'#streaming-chart'</span>)
     .datum(data)
     .call(chart);
}
...
renderChart();
<span class="hljs-keyword">var</span> intervalId = setInterval(renderChart, <span class="hljs-number">200</span>);
</code></pre>
